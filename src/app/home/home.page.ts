...
export class HomePage {
  ...
  async getAddress(home) {
    const res: any = await this.http
      .get(
        `https://api.tomtom.com/search/2/reverseGeocode/${home.latitude}%2C${home.longitude}.json?key=${this.key}`
      )
      .toPromise();
  ...
  async search() {
    ...
    const res: any = await this.http
      .get(
        `https://api.tomtom.com/search/2/search/${this.query}.json?lat=${coordinates.coords.latitude}&lon=${coordinates.coords.longitude}&key=${this.key}`
      )
      .toPromise();
